...
class Cart:
    ...
    def new(self, request):
        if request.user.is_authenticated:
            ...
            user = None
        cart = models.Cart.objects.create(creation_date=datetime.datetime.now(), user=user)
    ...
    def update(self, product, unit_price, quantity):
        ...
        if item:
            ...
            self.add(product, quantity, unit_price)

    def count(self):
        return self.cart.item_set.all().aggregate(Sum('quantity')).get('quantity__sum', 0)

    def summary(self):
        return self.cart.item_set.all().aggregate(total=Sum(F('quantity')*F('unit_price'), output_field=MoneyField())).get('total', 0)
